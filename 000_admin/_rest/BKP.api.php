    ...
    if(isset($_GET['action']) && $_GET['action']){
        ...
        if($action == 'buscarRefSelect'){
            ...
            if(isset($_GET['q'])){
                ...
                FROM ".$idioma."_repuestos where noRefFuster LIKE '%$busqueda%' LIMIT 50) order by text asc;");
            }else{
                $query = $cxn_bd->prepare("(SELECT referencia as 'id', referencia as 'text' 
                FROM refoem JOIN rel_repuestos_refoem ON refoem.id = rel_repuestos_refoem.idRefOem LIMIT 50)
                UNION 
                (SELECT noRefFuster as 'id', noRefFuster as 'text' 
                FROM ".$idioma."_repuestos LIMIT 50) order by text asc; ");
            }
            // if(isset($_GET['q'])){
            //     $busqueda = trim($_GET['q']);
            //     $query = $cxn_bd->prepare("SELECT referencia as 'id', referencia as 'text' 
            //     FROM refoem JOIN rel_repuestos_refoem ON refoem.id = rel_repuestos_refoem.idRefOem 
            //     where referencia LIKE '%$busqueda%'
            //     UNION 
            //     SELECT noRefFuster as 'id', noRefFuster as 'text' 
            //     FROM ".$idioma."_repuestos where noRefFuster LIKE '%$busqueda%' order by CAST(text as INT) asc LIMIT 50;");
            // }else{
            //     $query = $cxn_bd->prepare("SELECT referencia as 'id', referencia as 'text' 
            //     FROM refoem JOIN rel_repuestos_refoem ON refoem.id = rel_repuestos_refoem.idRefOem 
            //     UNION 
            //     SELECT noRefFuster as 'id', noRefFuster as 'text' 
            //     FROM ".$idioma."_repuestos order by CAST(text as INT) asc LIMIT 20; ");
            // }
            if($query){
                if($query->execute()){
                    $queryData = $query->get_result();
        ...
        if($action == 'buscarByRefOem'){
            ...
            LEFT JOIN rel_repuestos_imagenes as relimg on relimg.idRepuesto = re.id
            LEFT  JOIN ".$idioma."_imagenes on ".$idioma."_imagenes.id = relimg.idImagen
            WHERE (refoem.referencia = '$refOem' OR re.noRefFuster= '$refOem' OR re.refComp LIKE '%$refComp1%' OR re.refComp LIKE '%$refComp2%' OR re.refComp LIKE '%$refComp3%') and re.tipo = 1  and rel4.tipoProd = 1 and relimg.principal = 1
            and (
        ...
        if($action == 'buscarByRefOemConjunto'){
            ...
            for ($i=1; $i < count($refOem) ; $i++) { 
                ...
                JOIN rel_repuestos_imagenes as relimg on relimg.idRepuesto = re.id
                JOIN ".$idioma."_imagenes on ".$idioma."_imagenes.id = relimg.idImagen
                WHERE (refoem.referencia = '$ref' OR re.noRefFuster= '$ref' OR re.refComp LIKE '%$refComp1%' OR re.refComp LIKE '%$refComp2%' OR re.refComp LIKE '%$refComp3%') and re.tipo = 1  and rel4.tipoProd = 1 GROUP BY re.id;
                ");
        ...
        if($action == 'buscarByCateMarcMode'){
            ...
            LEFT JOIN rel_repuestos_imagenes as relimg on relimg.idRepuesto = re.id
            LEFT  JOIN ".$idioma."_imagenes on ".$idioma."_imagenes.id = relimg.idImagen
            WHERE rel.idModelo IN ($modelo) and marcas.id = $marca and ".$idioma."_categorias.id = $categoria and re.tipo = 1  and rel4.tipoProd = 1 group by re.id) as listado JOIN ".$idioma."_categorias as cat2 ON listado.idP = cat2.id;
            ");
