...
$(document).ready(function($) {
	...
	$.ajax({
		method: "GET",
		url: "../../000_admin/_rest/api.php?action=comprobarTipoAccesorioList&cat="+idCategoria
	...
	function cargaVistaNormal(){
		$.ajax({
			method: "GET",
			url: "../../000_admin/_rest/api.php?action=listarAccesoriosByCategoria&cat="+idCategoria
		}).done(function(response) {
			if(response){
				var data = JSON.parse(response);
				var result = data.result;
				...
				if(data.code == 1 && result.length > 0){
					...
					result.forEach(function(row, index) {
						...
						$.ajax({
							method: "GET",
							url: "../../000_admin/_rest/api.php?action=listarCaractByIdAccesorio&cat="+idCategoria+"&idProd="+id
						}).done(function(response) {
							if(response){
								p++;
								var data = JSON.parse(response);
													...
													<div class="bloque_detalle">\
														<div class="field-content">\
															<h4 class="titulo-producto">\
																...
																<span class="precioProductos"> 8.42 €  </span>\
															</h4>\
														</div>\
	...
	function cargaVistaTabla(){	
		...
		$.ajax({
			...
			url: "../../000_admin/_rest/api.php?action=buscarAccesoriosByIdProducto"
		}).done(function(response) {
			var p = 0;
			var contenido = '';
			var listadoFiltros = '';
						...
						$.ajax({
							...
							if(response){
								...
								if(p == result.length){
									...
									//console.log('arraycol1', arraycol1)
									...
									for (let index99 = 0; index99 < arraytd.length; index99++) {
										...
										for (let i = 0; i < ordenCaracteristicas.length; i++) {
											...
											for (let j = 0; j < caractElement.length; j++) {
												...
												if(caractElement[j].id == orden){
													...
													listadoCaracteristicas += `<td class='tecni idC`+caractElement[j].id+`  `+datosReferencia[index99].noRefFuster+`   ' data-value='`+valorCaract+`'  data-id='`+datosReferencia[index99].noRefFuster+`'>`+valorCaract+`</td>`;
													encontro = true;
												}
												...
												if(j == caractElement.length-1){
													if(!encontro)
													{
														listadoCaracteristicas += `<td class='tecni idC-' data-value='-'  data-id='`+datosReferencia[index99].noRefFuster+`'>-</td>`;
													}
												}
											}
										...
										listadoAccesorios += '\
										<tr id="'+datosReferencia[index99].noRefFuster+'" style="display: table-row;">\
											<td class="refer">\
